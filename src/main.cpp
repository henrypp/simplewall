...
size_t _app_getappgroup (size_t hash, PITEM_APP const ptr_app)
{
	...
	if (app.ConfigGet (L"IsEnableSpecialGroup", false).AsBool () && _app_apphaverule (hash))
		return 1;
...
bool _wfp_logsubscribe ()
{
	...
	else
	{
		...
		else
		{
			...
			if (!_FwpmNetEventSubscribe3 && !_FwpmNetEventSubscribe2 && !_FwpmNetEventSubscribe1 && !_FwpmNetEventSubscribe0)
			{
