	...
	if (ptr_log_object)
	{
		...
		if (ptr_log)
		{
			...
			// print table text
			{
				...
				_app_notifysettext (hdc, hwnd, IDC_PORT_ID, app.LocaleString (IDS_PORT, L":"), IDC_PORT_TEXT, ptr_log->port ? _r_fmt (L"%d (%s)", ptr_log->port, _app_getportname (ptr_log->port).GetString ()) : empty_text);
